...
class EmbeddingGraphBranch(nn.Module):
    ...
    def forward(self, graph_batch, return_tokens=False):
        ...
        # Graph pooling on valid nodes only.
        mask_f = mask.unsqueeze(-1).float()
        node_count = mask_f.sum(dim=1).clamp_min(1.0)
        graph_feat = (x_dense * mask_f).sum(dim=1) / node_count

        if return_tokens:
            return graph_feat, x_dense, mask
        return graph_feat
