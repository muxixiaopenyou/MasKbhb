...
def save_loss_history_csv(rows, save_path):
    ...
    with open(save_path, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'train_loss', 'val_loss', 'val_score', 'lr'])
...
def main():
    ...
    logger.info(f"LR Scheduler: ReduceLROnPlateau(factor={lr_scheduler_factor}, patience={lr_scheduler_patience}, min_lr={min_lr})")
    logger.info("="*30)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(x_dummy, y_for_cv)):
        ...
        patience_counter = 0
        save_path = f"./models/fold_{fold+1}.pth"
        ...
        history_csv = f"./models/fold_{fold+1}.csv"
        save_loss_history_csv(loss_rows, history_csv)
        history_png = f"./models/fold_{fold+1}.png"
        saved_png = save_loss_curve_png(
            loss_rows,
            history_png,
            title=f"Fold {fold+1} Loss Curve"
        )
