...
from model_windowsize import *
from utils import load_fasta

WINDOW_SIZES = [21, 31, 41, 51, 61]
...
def run_window_size_experiment(ws, batch_size, lr, weight_decay, n_epochs, k_folds,
    ...
    logger.info(f"  Sequence Window Size: {ws}")
    logger.info(f"  Sequence Center Index: {SEQ_CENTER_IDX}")
    ...
    train_graph_path = resolve_data_path(
        data_dir,
        [f'train_{DATA_TAG}_embedding_graph.pt']
    )
    test_graph_path = resolve_data_path(
        data_dir,
        [f'test_{DATA_TAG}_embedding_graph.pt']
    )
    ...
    train_fasta = resolve_data_path(data_dir, [f'train_{DATA_TAG}.fasta'])
    test_fasta = resolve_data_path(data_dir, [f'test_{DATA_TAG}.fasta'])
    train_ids, train_seqs, _ = load_fasta(train_fasta)
    test_ids, test_seqs, _ = load_fasta(test_fasta)
    validate_sequence_lengths(train_ids, train_seqs, ws, train_fasta)
    validate_sequence_lengths(test_ids, test_seqs, ws, test_fasta)
    ...
    for fold, (train_idx, val_idx) in enumerate(group_kfold.split(x_dummy, y_for_cv, groups)):
        ...
        patience_counter = 0
        save_path = f"{models_dir}/fold_{fold+1}_{DATA_TAG}.pth"
        ...
        for epoch in range(n_epochs):
            ...
            if patience_counter >= patience:
                logger.info(
                    ...
                    f"(best {early_stop_metric}={best_val_score:.4f})"
                )
                break
    ...
    test_ids_list = test_dataset.ids
    save_predictions(test_ids_list, y_test_true, ensemble_probs, ensemble_preds,
...
def main():
    ...
    for ws in WINDOW_SIZES:
        result = run_window_size_experiment(
            ws=ws,
            batch_size=batch_size,
            lr=lr,
    ...
    with open(summary_path, 'w', encoding='utf-8') as sf:
        ...
        slog("=" * 70)
        slog("FINAL COMPARISON: WINDOW SIZE vs PERFORMANCE")
        slog("=" * 70)

        metric_names = ['MCC', 'SN', 'SP', 'ACC', 'BACC', 'PRECISION', 'F1', 'AUC', 'AUCPR']

        slog("\n--- Cross-Validation Results (mean +/- std across folds) ---")
        header = f"{'WS':<6}" + "".join(f"{m:>14}" for m in metric_names)
        slog(header)
        slog("-" * len(header))
        ...
        header = f"{'WS':<6}" + "".join(f"{m:>10}" for m in metric_names)
        slog(header)
        slog("-" * len(header))
        for r in all_results:
            ws = r['window_size']
