...
def run_trans_experiment(n_layers, batch_size, lr, weight_decay, n_epochs, k_folds,
                       patience, early_stop_metric, early_stop_min_delta, seed,
                       lr_scheduler_factor, lr_scheduler_patience, min_lr, device):
    LAYER_TAG = f'trans{n_layers}'
    data_dir = 'data'

    model_module = importlib.import_module(f'model_{LAYER_TAG}')

    models_dir = f'models_{LAYER_TAG}'
    os.makedirs(models_dir, exist_ok=True)

    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_filename = f'{models_dir}/training_{current_time}_{LAYER_TAG}.log'
    ...
    logger.info("=" * 60)
    logger.info(f"TRANSFORMER LAYERS: {n_layers}")
    logger.info("=" * 60)
    ...
    for fold, (train_idx, val_idx) in enumerate(group_kfold.split(x_dummy, y_for_cv, groups)):
        ...
        model = model_module.PLMGraphModel(
            node_dim=1280, edge_dim=16, hidden_dim=128, num_classes=2
        ).to(device)
    ...
    for name in metric_names:
        ...
        logger.info(f"  {name:10}: {mean_val:.3f} ± {std_val:.3f}")
    ...
    for path in fold_paths:
        eval_model = model_module.PLMGraphModel(
            node_dim=1280, edge_dim=16, hidden_dim=128, num_classes=2
        ).to(device)
    ...
    save_predictions(test_ids, y_test_true, ensemble_probs, ensemble_preds,
                     f'{models_dir}/test_predictions_{LAYER_TAG}.csv')
...
def main():
    ...
    for n_layers in TRANS_LAYERS:
        result = run_trans_experiment(
            n_layers=n_layers,
            batch_size=batch_size,
            lr=lr,
    ...
    with open(summary_path, 'w', encoding='utf-8') as sf:
        ...
        slog("=" * 70)
        slog("FINAL COMPARISON: TRANSFORMER LAYERS vs PERFORMANCE")
        slog("=" * 70)

        metric_names = ['MCC', 'SN', 'SP', 'ACC', 'BACC', 'PRECISION', 'F1', 'AUC', 'AUCPR']

        slog("\n--- Cross-Validation Results (mean +/- std across folds) ---")
        header = f"{'Layers':<8}" + "".join(f"{m:>14}" for m in metric_names)
        slog(header)
        slog("-" * len(header))
        ...
        header = f"{'Layers':<8}" + "".join(f"{m:>10}" for m in metric_names)
        slog(header)
        slog("-" * len(header))
        for r in all_results:
            n = r['n_layers']
